...
if (ord(t)=1,
	p70_mow_yld_corr(j) = 1;
	p70_mow_yld_corr(j)$((im_past_yields(t,j,"mowing","rainfed") * pm_land_start(j,"past")) > 0) =
											 (sum(cell(i,j),p70_total_past_demand(t,i)) * p70_lsus_dist_weight(t,j)) /
											 (im_past_yields(t,j,"mowing","rainfed") * pm_land_start(j,"past"));
	p70_mow_yld_corr(j)$(p70_mow_yld_corr(j) < 1)  = 1;
 	);

im_past_yields(t,j,"mowing","rainfed") = im_past_yields(t,j,"mowing","rainfed") * p70_mow_yld_corr(j);
