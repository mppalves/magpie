...
if (sum(sameas(t_past,t),1) = 1,
	p70_mow_yld_corr(i,j) = 1;
	p70_mow_yld_corr(i,j)$(sum(cell(i,j),im_past_yields(t,j,"mowing","rainfed") * pm_land_hist(t,j)) > 0) =
											 p70_total_past_demand(t,i) /
											 sum(cell(i,j), im_past_yields(t,j,"mowing","rainfed") * pm_land_hist(t,j));
	p70_mow_yld_corr(i,j)$(p70_mow_yld_corr(i,j) < 1)  = 1;
 	);

im_past_yields(t,j,"mowing","rainfed") = (im_past_yields(t,j,"mowing","rainfed") * sum(cell(i,j), p70_mow_yld_corr(i,j))$(sum(sameas(t_past,t),1) = 1) +
sum(t_past,(im_past_yields(t,j,"mowing","rainfed") * sum(cell(i,j), p70_mow_yld_corr(t_past,j)))$(ord(t_past)=card(t_past)))$(sum(sameas(t_past,t),1) <> 1);
