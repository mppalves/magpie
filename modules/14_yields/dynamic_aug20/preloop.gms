...
p14_pyield_LPJ_reg(t,i) = (sum(cell(i,j),im_past_yields(t,j,"mowing","rainfed")*pm_land_start(j,"past"))/sum(cell(i,j),pm_land_start(j,"past")) );
...
im_past_yields(t,j,"mowing",w) = im_past_yields(t,j,"mowing",w)*sum(cell(i,j),p14_pyield_corr(t,i));

***YIELD CALIBRATION***********************************************************************
i14_yields(t,j,kcr,w)       = i14_yields(t,j,kcr,w)      *sum(cell(i,j),f14_yld_calib(i,"crop"));
im_past_yields(t,j,"mowing",w) = im_past_yields(t,j,"mowing",w)*sum(cell(i,j),f14_yld_calib(i,"past"));
