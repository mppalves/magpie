...
* Mind that preloop adjustments are made only on continous pasture for now.
*################################# DEVELOPMENT #################################
...
p14_pyield_LPJ_reg(t,i) = (sum(cell(i,j),i14_past_yields(t,j,"pasture","rainfed")*pm_land_start(j,"past"))/sum(cell(i,j),pm_land_start(j,"past")) );
...
i14_past_yields(t,j,"pasture",w) = i14_past_yields(t,j,"pasture",w)*sum(cell(i,j),p14_pyield_corr(t,i));

***YIELD CALIBRATION***********************************************************************
i14_yields(t,j,kcr,w)       = i14_yields(t,j,kcr,w)      *sum(cell(i,j),f14_yld_calib(i,"crop"));
i14_past_yields(t,j,"pasture",w) = i14_past_yields(t,j,"pasture",w)*sum(cell(i,j),f14_yld_calib(i,"past"));
