...
p14_grass_corr(t,i,past_mngt) = (f14_grassl_yld_hist(t,i,past_mngt)/(p14_grass_yields(t,i,past_mngt) + 1e-9))$(sum(sameas(t_past,t),1) = 1) +
                                sum(t_past,(f14_grassl_yld_hist(t_past,i,past_mngt)/(p14_grass_yields(t_past,i,past_mngt) + 1e-9))$(ord(t_past)=card(t_past)))$(sum(sameas(t_past,t),1) <> 1);
p14_grass_corr(t,i,past_mngt)$(p14_grass_corr(t,i,"pastr") < 1) = 1;
...
p14_grass_yields(t_past,i,past_mngt)
   ...
   + (sum(cell(i,j),i14_grassland_total(t_past,j) * i14_grass_yields(t_past,j,past_mngt,"rainfed")) /
      sum(cell(i,j),i14_grassland_total(t_past,j)))$(sum(cell(i,j), fm_LUH2v2(t_past,j,past_mngt))=0);

loop(t,
     if(sum(sameas(t,"y1995"),1)=1,

          if    ((s14_limit_calib = 0),
               i14_lambda_grass(t,i,past_mngt) = 1;

          Elseif (s14_limit_calib =1 ),
               i14_lambda_grass(t,i,past_mngt) =
                    1$(f14_grassl_yld_hist(t,i,past_mngt) <= p14_grass_yields(t,i,past_mngt))
                    + sqrt(p14_grass_yields(t,i,past_mngt)/f14_grassl_yld_hist(t,i,past_mngt))$
                    (f14_grassl_yld_hist(t,i,past_mngt) > p14_grass_yields(t,i,past_mngt));
          );

          i14_grassl_yld_hist_reg(t,i,past_mngt) = f14_grassl_yld_hist(t,i,past_mngt);

     Else
          p14_grass_yields(t,i,past_mngt) = p14_grass_yields(t-1,i,past_mngt);
          i14_grassl_yld_hist_reg(t,i,past_mngt)  = i14_grassl_yld_hist_reg(t-1,i,past_mngt);
          i14_lambda_grass(t,i,past_mngt)   = i14_lambda_grass(t-1,i,past_mngt);
     );
);


*' The calibrated cellular yield 'i14_yields_calib' is calculated for each time step depending
*' on the constant values 'p14_grass_yields', 'i14_grassl_yld_hist_reg', 'i14_lambda_grass'
*' and the uncalibrated, cellular yield 'f14_yields' following the idea of eq. (9) in @Heinke.2013:

p14_grass_corr(t,j,past_mngt) =
  1 + (sum(cell(i,j), i14_grassl_yld_hist_reg(t,i,past_mngt) - p14_grass_yields(t,i,past_mngt)) /
                             f14_grassl_yld(t,j,past_mngt,"rainfed") *
