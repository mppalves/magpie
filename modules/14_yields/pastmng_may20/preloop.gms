...
f14_pyld_hist_cell(t,j) = sum(cell(i,j),f14_pyld_hist(t,i))

p14_pyield_corr(t,j) = (f14_pyld_hist_cell(t,j)/p14_pyield_LPJ(t,j))$(sum(sameas(t_past,t),1) = 1)
			+ sum(t_past,(f14_pyld_hist_cell(t_past,j)/(p14_pyield_LPJ(t_past,i)+0.000001))$(ord(t_past)=card(t_past)))$(sum(sameas(t_past,t),1) <> 1);
...
vm_lsu.l(t, j) = f14_initial_lsu(t,j);
