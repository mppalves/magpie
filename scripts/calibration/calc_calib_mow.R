...
get_costcalib <- function(gdx_file) {
  require(magclass)
  require(gdx)
  calib_mow_cost <- readGDX(gdx_file, "i31_mow_cost")
...
update_calib_cost_mowing <- function(gdx_file,
  ...
  area_factor <- get_areacalib(gdx_file)[, 1, ]
  tc_factor <- get_yieldcalib(gdx_file)[, 1, ]
  cost_factor <- get_costcalib(gdx_file)[, 1, ]
  calib_correction <- area_factor * tc_factor
  calib_divergence <- abs(calib_correction - 1)
  calib_divergence <- add_columns(calib_divergence, addnm = "cmow", dim = 3.1)
  calib_divergence[, , "cmow"] <- abs(area_factor[, , "past"] - 1)

  old_calib <- magpiesort(read.magpie(calib_file))
  calib_factor_cmow <- old_calib[, , "cmow"] * (damping_factor * (area_factor[, , "past"] - 1) * cost_factor + 1)
  getNames(calib_factor_cmow) <- "cmow"
  calib_factor_crop <- old_calib[, , "crop"] * (damping_factor * (calib_correction[, , "crop"] - 1) + 1)
  calib_factor <- mbind(calib_factor_crop, old_calib[, , "past"], calib_factor_cmow)
  ...
  write_log <- function(x, file, calibration_step) {
    ...
    try(write.magpie(round(setYears(x, NULL), 2), file, append = (calibration_step != 1), comment = "Mowing cost Calibration"))
  }
  ...
  write_log(tc_factor[, , "crop"], "calib_tc_factor.cs3", calibration_step)
...
# Calculate the correction factor and save it
update_calib <- function(gdx_file,
                         calib_accuracy = 0.1,
                         calibrate_pasture = TRUE,
                         calibrate_cropland = TRUE,
                         damping_factor = 0.8,
                         calib_file,
                         crop_max = 1,
  ...
  old_calib <- magpiesort(read.magpie(calib_file))
  calib_factor <- old_calib[, , c("crop", "past")] * (damping_factor * (calib_correction - 1) + 1)
  calib_factor <- mbind(calib_factor, old_calib[, , "cmow"])
  ...
  for (i in 1:n_maxcalib) {
    ...
    if (calibrate_cost_mowing) {
        ...
        update_calib(
          gdx_file = paste0(putfolder, "/fulldata.gdx"),
          calib_accuracy = calib_accuracy,
          calibrate_pasture = calibrate_pasture,
          calibrate_cropland = calibrate_cropland,
          damping_factor = damping_factor,
          calib_file = calib_file,
          calibration_step = i
