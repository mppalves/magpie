...
start_run <- function(cfg,scenario=NULL,codeCheck=TRUE,
  ...
  if(!file.exists(calib_file) | !file.exists(mngmt_cost_file)) stop("Yield calibration file missing!")
  ...
  if(cfg$recalibrate){
    ...
    source("scripts/calibration/calc_calib_backup.R")
    calibrate_magpie(n_maxcalib = cfg$calib_maxiter,
                     ...
                     #calibrate_past_mngt_cost = (cfg$gms$yields=="management_mar21"),
                     calibrate_past_mngt_cost = TRUE,
